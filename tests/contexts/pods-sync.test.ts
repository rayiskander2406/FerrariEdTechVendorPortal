...
describe("PoDS API Sync Endpoint", () => {
  ...
  it("GET /api/pods without vendorName should return all applications", async () => {
    ...
    await addPodsApplication(
      createMockPodsApplication({ vendorName: "Vendor A" })
    );
    await addPodsApplication(
      createMockPodsApplication({ vendorName: "Vendor B" })
    );
...
describe("PoDS Stale Cache Scenario", () => {
  ...
  it("should handle multiple vendors in cache", async () => {
    ...
    const response1 = await GET(
      new Request(
        `http://localhost:3000/api/pods?vendorName=${encodeURIComponent("Vendor One")}`
      ) as any
    );
    const data1 = await response1.json();
    expect(data1.application.status).toBe("APPROVED");

    const response2 = await GET(
      new Request(
        `http://localhost:3000/api/pods?vendorName=${encodeURIComponent("Vendor Two")}`
      ) as any
    );
    const data2 = await response2.json();
    expect(data2.application.status).toBe("EXPIRED");
  });
});
